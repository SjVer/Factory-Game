...
func on_save_clicked(save: Dictionary):
	selected_save = save
	print("selected: %s" % save["path"])
	
	name_label.text = save["name"]
	seed_label.text = str(save["seed"]) + "    "
...
func _on_Copy_pressed():
	print("copying seed %d" % selected_save["seed"])
	OS.set_clipboard(str(selected_save["seed"]))

func _on_ButtonDelete_pressed():
	# make sure we can delete the selected file
	if selected_save.has("path"):
		# Warning first
